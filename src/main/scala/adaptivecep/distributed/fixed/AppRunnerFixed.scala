...
object AppRunnerFixed extends App {
  ...
  val publisherA: ActorRef = actorSystem.actorOf(Props(RandomPublisher(id => Event1(id))).withDeploy(Deploy(scope = RemoteScope(publisherAddress1))),             "A")
  val publisherB: ActorRef = actorSystem.actorOf(Props(RandomPublisher(id => Event1(id * 2))).withDeploy(Deploy(scope = RemoteScope(publisherAddress2))),         "B")
  val publisherC: ActorRef = actorSystem.actorOf(Props(RandomPublisher(id => Event1(id.toFloat))).withDeploy(Deploy(scope = RemoteScope(publisherAddress3))),     "C")
  val publisherD: ActorRef = actorSystem.actorOf(Props(RandomPublisher(id => Event1(s"String($id)"))).withDeploy(Deploy(scope = RemoteScope(publisherAddress4))), "D")
  ...
  val query2: Query4[Int, Int, Float, String] =
    stream[Int]("A")
      ...
      .join(
        ...
        slidingWindow(3.seconds),
        latency < timespan(2.milliseconds) otherwise { (nodeData) => println(s"PROBLEM:\tEvents reach node `${nodeData.name}` too slowly!") })
  ...
  val placement: ActorRef = actorSystem.actorOf(Props(PlacementActorFixed(actorSystem,
    ...
    PathLatencyMonitorFactory(interval =  5, logging = true), NodeHost(host14), hosts)), "Placement")
