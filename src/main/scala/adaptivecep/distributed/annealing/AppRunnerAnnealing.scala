  ...
  val query3: Query4[Int, Int, Float, String] =
    stream[Int]("A")
      ...
      .and(stream[Float]("C").and(stream[String]("D")),
        ...
        /*frequency > ratio(30.instances, 1.seconds) otherwise { nodeData => /*println(s"PROBLEM:\tNode `${nodeData.name}` emits too few events!")*/ }*/)


  val query4: Query3[Int, Int, Float] =
    stream[Int]("A")
      .join(
        stream[Int]("B"),
        slidingWindow(2.seconds),
...
*/
  val host1: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address1))), "Host" + "1")
  val host2: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address2))), "Host" + "2")
  val host3: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address3))), "Host" + "3")
  val host4: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address4))), "Host" + "4")
  val host5: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address5))), "Host" + "5")
  val host6: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address6))), "Host" + "6")
  val host7: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address7))), "Host" + "7")
  val host8: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address8))), "Host" + "8")
  val host9: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address9))), "Host" + "9")
  val host10: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address10))), "Host" + "10")
  val host11: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address11))), "Host" + "11")
  val host12: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address12))), "Host" + "12")
  val host13: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address13))), "Host" + "13")
  val host14: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address14))), "Host" + "14")
  val host15: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address15))), "Host" + "15")
  val host16: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address16))), "Host" + "16")
  val host17: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address17))), "Host" + "17")
  val host18: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address18))), "Host" + "18")
  val host19: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address19))), "Host" + "19")
  val host20: ActorRef = actorSystem.actorOf(Props[HostActorAnnealing].withDeploy(Deploy(scope = RemoteScope(address20))), "Host" + "20")
